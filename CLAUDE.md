...
| [docs/deployment.md](docs/deployment.md) | 从零起新集群清单 + secret/vars 全表（部署方视角；issue #574） |
...
### Issue 立项纪律（dogfood 期）

跑真链路撞坑时**默认进 [BACKLOG.md](BACKLOG.md)，不立 issue**。立 issue 的门槛：
- ≥3 次同类 hit（playbook §8 cap）
- 或阻塞核心定位（P0/P1 接入面 / 跨仓 root cause）

理由：立 issue = 隐性承诺修；BACKLOG.md = 只观察。一个人 + AI 项目最大坑是"AI 产能放大让 self-fix backlog 比 dogfood 跑得快"。

详见 [docs/playbook.md §14](docs/playbook.md)。

### Dogfood 期间 AI 协作红线（最高优先级，AI 必读）

> 0.x 阶段 user 在跑全链路 dogfood。**目标是把 ttpos REQ 推到下一段，不是把 sisyphus 修对**。AI 老把 user 拐去修 bug，本节防止这条。

#### AI **不允许** 做的

1. ❌ 建议 manual fix sisyphus orch bug（除非同一洞 ≥2 条 REQ 撞过 = 阻塞类）
2. ❌ 建议派 REQ 修 sisyphus 自身 / 派 fixer agent / 拆 sub-issue
3. ❌ 建议改 sisyphus prompt / state machine / verifier / checker
4. ❌ 写新 retrospective / 契约设计 / playbook 章节（除 user 明确要求）
5. ❌ 把"撞了 1 次的洞"当 pattern 提议修
6. ❌ 给"分阶段 / Tier 1/2/3 / Phase A/B/C"路线图（这是修 bug 的伪装）

#### AI **应该** 做的

| 场景 | AI 该说的 |
|---|---|
| user 撞 1 个洞 | "挂 issue（或 BACKLOG.md log），跳过这条 REQ，派下一条" |
| 同一洞撞 ≥2 次 | "minimal hack 让它过（30s 内 ssh / kubectl 干完，不入代码），不修对，继续" |
| user 问"该不该修这个 bug" | "≥3 次同类 hit 才修，否则只 log" |
| user 问"我思路是不是错了" | 先看数据（issue 列表 / git log）再答；不要泛 retrospective |
| sisyphus pipeline 自动 emit issue | "默认不立刻处理，看一周累计 pattern" |
| user 问"要不要改 sisyphus" | "这周不改。判据：5 条 ttpos REQ 端到端跑过期间 sisyphus 主链 0 改动 = v0.x stable" |

#### AI 自检（每次回应前过一遍）

回应字数 > 200 字 / 出现 ≥3 次"建议"或"应该" / 引用 ≥2 条新 issue / 给路线图 ——
**多半在拐 user 修 bug**。压缩到 ≤100 字 + 1 个具体动作。

#### 唯一真问题

> 这条建议让 user 更接近第 1 条 ttpos REQ 跑通吗？

不是 → defer。是 → 给最 dirty 最快的做法，不要"修对"。

详见 [docs/playbook.md §15 Dogfood 操作手册](docs/playbook.md)。
