...
人 ──intent:analyze──▶ analyze-agent (写 proposal/design/tasks)
     ...
     verifier ◀── pr-ci-watch (sisyphus 直调 GitHub REST 轮 check-runs)
        ...
        │   pass ────┘
        │            ▼
        │         accept-agent (sisyphus 先 make ci-accept-env-up，agent 跑 FEATURE-A*)
        │            │
        │   pass ────┘
        ▼            ▼
       fixer       teardown (make ci-accept-env-down，必跑)
        │            │
...
| `make ci-accept-env-up` | sisyphus pre-accept | 起 lab（helm install / docker compose up） |
| `make ci-accept-env-down` | teardown_accept_env | accept 完必跑（best-effort 幂等） |
