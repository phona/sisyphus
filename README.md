...
Sisyphus 不写代码、不审内容、不抢 AI 决定权。它是一层薄薄的"调度 + 机械 checker + 度量"，让 execute / spec / dev / accept 这些 agent 各自把活干完，并且**跑得起来、跑得稳、能被指标驱动改进**。

## 三个核心价值

1. **自愈** —— stage 失败不直接停，verifier-agent 主观判 pass / fix / retry / escalate；fixer agent 自动闭环；watchdog 兜底卡死。
2. **范式强制** —— PR 必须真开、useWorktree 默认开；mechanical checker（staging-test / pr-ci / accept env）独立于 agent 报告，sisyphus 是唯一裁判。
3. **观测指导改进** —— 每个 stage 入 `stage_runs`、每条 verifier 决策入 `verifier_decisions`，22 条 Metabase SQL（Q1–Q22）回答"哪条 prompt 该改 / 哪个 stage 钻牛角尖 / 哪类 fixer 修不动 / 机械 checker 有没有沉默通过 / 多少 token 被废弃 REQ 浪费"。
...
```
人 ──intent:execute──▶ execute-agent (全责交付：写 spec + 业务码 + push feat/REQ-x + 开 PR；自决拆 sub-issue)
                         │
                         ▼
            spec-lint (机械: openspec validate + check-scenario-refs.sh)
                         │
                         ▼
            challenger-agent (M18: 黑盒读 spec 写 contract test, 推 feat 分支)
                         │
                         ▼
            dev-cross-check (机械: BASE_REV make ci-lint, 仅 lint 变更文件)
                         │
                         ▼
        ┌── staging-test (sisyphus kubectl exec runner pod 跑 make ci-unit-test && make ci-integration-test)
        │            │
        │   pass ────┘
        │            │
        ▼            ▼
     verifier ◀── pr-ci-watch (sisyphus 直调 GitHub REST 轮 check-runs)
        ...
        │   pass ────┘
        │            ▼
        │         accept-agent (sisyphus 先 make accept-env-up，agent 跑 FEATURE-A*)
        │            │
        │   pass ────┘
        ▼            ▼
       fixer       teardown (make accept-env-down，必跑)
        │            │
        └────────────▶ archive (各仓 openspec apply 归档；不自动合 PR)
```
...
│   ├── src/orchestrator/
│   │   ├── state.py          # 状态机 + 事件枚举（17 ReqState × 27 Event）
│   │   ├── router.py         # webhook → Event 翻译
│   │   ├── engine.py         # 事件分发 + action 执行
│   │   ├── webhook.py        # BKD webhook 入口
│   │   ├── actions/          # stage 推进动作（start_intake / start_execute / start_challenger / create_spec_lint / create_dev_cross_check / create_staging_test / create_pr_ci_watch / create_accept / teardown_accept_env / done_archive / escalate / ...）
│   │   ├── checkers/         # 4 个机械 checker：spec_lint / dev_cross_check / staging_test / pr_ci_watch
│   │   ├── prompts/          # Jinja2 模板（stage agent + 14 对 verifier/{stage}_{trigger}.md.j2）
...
├── observability/
│   ├── schema.sql
│   ├── queries/sisyphus/     # 22 条 Metabase SQL（Q1–Q22：M7 + M14e + fixer-audit Q14–Q16 + dedup-retry Q17 + silent-pass Q18 + termination Q21–Q22）
...
# 2. 触发一个 REQ（在 BKD 项目内开 issue + 打 intent:execute tag）
#    BKD 发 webhook → orchestrator router → start_execute action
...
| target | 谁调 | 用途 |
|---|---|---|
| `make ci-lint` | dev-cross-check checker | dev push 后跑（go vet + golangci-lint，仅变更文件，BASE_REV env） |
| `make ci-unit-test` | staging-test checker | 单元测试（业务聚合 main + bmp 等） |
| `make ci-integration-test` | staging-test checker | 集成测试（docker compose 起 stack） |
| `make accept-env-up` | sisyphus pre-accept | 起 lab（helm install / docker compose up） |
| `make accept-env-down` | teardown_accept_env | accept 完必跑（best-effort 幂等） |
...
| [docs/cookbook/](docs/cookbook/) | 按 lab 形态分别给 `accept-env-up/down` 实现样板（如 `ttpos-arch-lab-accept-env.md`：helm/K3s + emulator + APK + endpoint） |
