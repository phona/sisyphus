...
| tag | 触发 Event | 意义 |
|---|---|---|
| `intent:intake` | `INTENT_INTAKE` | 走完整流水线：先起 intake-agent 多轮澄清需求 → finalized intent JSON → 新建 execute issue 接力（推荐：不熟悉的仓） |
| `intent:execute` | `INTENT_EXECUTE` | 跳过 intake，直接进 execute-agent（适合 trivial REQ） |

router 看到 `intent:*` tag 且 issue 还没进 `execute` / `intake` 阶段（即 issue 上**没有**
...
| `execute` | execute-agent issue（M17 全责交付） |
...
|---|---|
| `verifier` | role tag |
| `verify:<stage>` | 这次 verify 的目标 stage（`execute` / `spec_lint` / `challenger` / `dev_cross_check` / `staging_test` / `pr_ci` / `accept`） |
...
| [`actions/start_execute.py`](../orchestrator/src/orchestrator/actions/start_execute.py) | `["execute", req_id, *forwarded]` |
| [`actions/start_execute_with_finalized_intent.py`](../orchestrator/src/orchestrator/actions/start_execute_with_finalized_intent.py) | `["execute", req_id, *forwarded]`（`sisyphus` 由 `BKDRestClient.create_issue` 自动加） |
...
| `spec_home_repo:<owner/repo>` | spec home 声明（M17 弱归属）。execute-agent / challenger-agent 据此挑哪仓写 spec |
