...
| **机械 checker** | 跑测试 / 轮 CI / 跑 ci-accept-env-up/down | Python, runner pod 内 exec | 只看 exit code / API 返回 |
...
| 7a | **accept env-up** (机械) | pr-ci pass | runner pod 跑 `make ci-accept-env-up`，stdout 尾行 JSON 取 `endpoint` | env-up 失败 → ESCALATED |
| 7b | **accept** | env-up 完 | 跑 FEATURE-A* scenarios → result:pass / fail tag | session.completed + accept tag |
| 8 | **teardown** (机械, 必跑) | accept 完（pass 或 fail） | `make ci-accept-env-down`，best-effort 失败只 warning | TEARDOWN_DONE_PASS / FAIL |
...
| **`make ci-accept-env-up` / `ci-accept-env-down`** | integration repo | sisyphus accept stage | 起 / 拆 lab |
...
| `SISYPHUS_STAGE` | ci-accept-env-up / ci-accept-env-down | 给业务 Makefile 区分阶段 |
...
- **接 ttpos-arch-lab 真 e2e** —— ci-accept-env-up / ci-accept-env-down 落到生产 lab
