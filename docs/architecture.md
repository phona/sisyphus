...
### 0.4 本 AI agent（我）的 push / merge 权限
...
**核心准则**：观测不是"看好看的图"，是**让每次改 prompt / 阈值能用数据验证效果**。`config_version` + `improvement_log` 两张表支撑"改动 → 度量"的**人工改进闭环**：人看 Metabase 看板发现异常 → 诊断根因 → 改 prompt → bump `config_version` → 等 2 周数据 → 回填 `verdict`。

> **为什么不自动化**：当前日处理量约 5 REQ，统计显著性不足；prompt 框架仍在快速迭代（M14→M18），过早自动化会打到移动靶上。自动化 IMPROVER daemon 需等数据量 ≥ 50 REQ/日且 prompt 结构稳定后再评估。
...
- **Prompt 自动改进（IMPROVER daemon）**：已 descope。`config_version` + `improvement_log` 表就绪，但改进闭环当前由人驱动。统计量够且 prompt 结构稳定后再评估自动化。
