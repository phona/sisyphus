...
极少数情况下，Flutter 仓本身想充当 integration repo —— 例如：只验后端 HTTP
协议、不需要 emulator UI 层；或者团队还没有 `ttpos-arch-lab` 同款 K3s lab。

> **完整食谱另起 cookbook**：[`docs/cookbook/ttpos-flutter-mobile-accept-env.md`](ttpos-flutter-mobile-accept-env.md)
> 单独覆盖 Flutter 仓自承 integration repo 的所有细节（mock backend 设计、
> `tests/docker-compose.accept.yml` 骨架、accept-agent scenario 限制、SISYPHUS_NAMESPACE
> 并发隔离、排查清单）。下面给一份**最小 Makefile 片段**作为索引示例。
...
accept-env-up:
	docker compose -p $(COMPOSE_PROJECT_NAME) -f $(ACCEPT_COMPOSE) up -d --wait >&2
	@port=$$(docker compose -p $(COMPOSE_PROJECT_NAME) -f $(ACCEPT_COMPOSE) \
	    port backend 8080 | awk -F: 'END{print $$NF}'); \
	if [ -z "$$port" ]; then \
	    echo "[ttpos-flutter-lab] FAIL: cannot resolve backend host port" >&2; exit 1; \
	fi; \
...
> option 只适合"只验 HTTP 层、不验 UI"的场景；决策树和 mock backend stack 详见
> [`ttpos-flutter-mobile-accept-env.md`](ttpos-flutter-mobile-accept-env.md)。
...
三份 cookbook **互补不替代**，各自覆盖流水线不同阶段的不同 repo 角色：

| | 本 cookbook | ttpos-flutter-mobile-accept-env | ttpos-arch-lab-accept-env |
|---|---|---|---|
| repo 角色 | **source repo**（Flutter 源仓） | **Flutter 仓自承 integration repo**（无 emulator） | **integration repo**（mobile e2e lab，有 emulator） |
| Makefile target | `ci-lint` / `ci-unit-test` / `ci-integration-test` | `accept-env-up` / `accept-env-down`（mock backend only） | `accept-env-up` / `accept-env-down`（helm + emulator） |
| sisyphus 调用阶段 | staging-test / dev-cross-check | accept（HTTP 黑盒） | accept（HTTP + UI flow） |
| APK 来源 | `ci-build`（可选） | ❌（不验 UI 不需要 APK） | 从 `/workspace/source/ttpos-flutter/` 编译 |
| emulator | 无 | 无 | 有（API 34 headless） |
| 适用 | 必备 | 过渡 / 团队没 arch-lab | 完整 mobile e2e 时首选 |
