...
## 10. Thanatos 验收能力层（accept stage 可选 opt-in）

[`docs/thanatos.md`](./thanatos.md) 定义的 thanatos 是 sisyphus accept stage 的
**可选**验收能力层 —— 业务仓建一个 `.thanatos/skill.yaml` 即启用，accept-agent
会走 thanatos MCP 路径而不是裸 curl。**不**建则等同未启用，accept stage 退到
curl fallback。**强 backward-compat**：sisyphus 自己（无 UI 仓）就用 curl path
继续跑，不用做任何改动。

### 10.1 业务仓 `.thanatos/` 目录约定

```
phona/<business-repo>/
├── .thanatos/
│   ├── skill.yaml         ← 必填（启用即必须）；schema 见 §10.2
│   ├── anchors.md         ← 可选；thanatos 写、agent commit 到 feat 分支
│   ├── flows.md           ← 同上
│   └── pitfalls.md        ← 同上
├── ...
└── openspec/changes/<REQ>/
    └── specs/<capability>/spec.md   ← #### Scenario: 块来源
```

`.thanatos/` 跟代码同 PR 进，不开第二个 review gate。anchors / flows / pitfalls
随 thanatos 探索沉淀，由 accept-agent 在 accept stage 自动 `git commit && git push`
到 `feat/<REQ>` 分支。

### 10.2 `skill.yaml` schema

权威 schema：[REQ-thanatos-m0-scaffold-v6 contract.spec.yaml](
../openspec/changes/REQ-thanatos-m0-scaffold-v6-1777283112/specs/thanatos/contract.spec.yaml)
`#/skill_yaml`。最小例子：

```yaml
name: pytoya-web
driver: playwright              # playwright | adb | http
entry: $ENDPOINT                # 占位符，thanatos 注入 accept-env-up endpoint
fixtures:
  admin_login:
    user: admin
    pass: admin
preflight:                      # 可选；M0 不强校验，M2+ driver 真用
  - assert: "a11y_node_count > 5"
```

### 10.3 `make accept-env-up` 的 endpoint JSON 增字段

启用 thanatos 时，业务仓 `make accept-env-up` 在 stdout **最后一行 JSON** 多吐：

```json
{"endpoint":"http://lab.accept-req-417.svc.cluster.local:8080",
 "namespace":"accept-req-417",
 "thanatos_pod":"thanatos-7b9f-xyz"}
```

| 字段 | 是否必需 | 说明 |
|---|---|---|
| `endpoint` | ✅ | 同 §3，accept-agent 主测目标 URL |
| `namespace` | （可选） | 跟 SISYPHUS_NAMESPACE 同值 |
| **`thanatos_pod`** | **启用 thanatos 时必须** | accept namespace 内 thanatos 容器所在 pod 名 |

`thanatos_pod` 缺失 → accept-agent 自动 fallback 到 curl path（无报错）。完整
helm install 样板见 [`docs/cookbook/ttpos-arch-lab-accept-env.md`](./cookbook/ttpos-arch-lab-accept-env.md) §6.5。

### 10.4 accept-agent 行为表

| `.thanatos/skill.yaml` 在 source repo | endpoint JSON `thanatos_pod` | accept-agent 行为 |
|---|---|---|
| 任一 source repo 有 | 字段存在 | **thanatos branch**：`kubectl exec <thanatos_pod> -- python -m thanatos run-scenario ...` per `#### Scenario:` |
| 任一 source repo 有 | 字段缺失 | curl fallback（业务仓 Makefile 没实现完整 thanatos install） |
| 全 source repo 都没 | 任意 | curl fallback（永远） |

不强制每仓启用 —— 单仓启用即整 REQ 走 thanatos。多仓部分启用是边角 case，M1
不优化（详见 design 决策 5）。
