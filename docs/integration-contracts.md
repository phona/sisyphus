...
> 涉及的 repo 信息直接写在 BKD intent issue description 里（人或 execute-agent 负责），
...
派一个 `intent:execute` + `repo:<org>/<target-repo>` tag 的 BKD issue，
...
### 2.4 execute-agent 推 PR 前 guard

execute-agent 在 `gh pr create` **之前**必须跑通：
...
**任一轮失败都禁止开 PR**。execute-agent 自己修、重跑、直到全绿。

常见失败场景：改了状态机或接口名 → 现有 contract / integration test assert 旧名 →
同步修测试 → 重跑 → 全绿后再 `gh pr create`。

这是 execute-agent "全责交付"契约的一部分——推上去的代码必须让 `make ci-*` 全绿。
...
"leader 仓"。execute-agent 在每仓写 proposal.md / tasks.md。
...
execute-agent 默认拆并行：每个被改的 source repo 起一个 `tag=dev + REQ-x`
...
## 10. Required tokens / secrets / vars

> 接入新业务仓的完整配置清单。每条附 **缺失症状** —— 出问题先对照这里。
> 本节只描述权限要求，不写任何真实 PAT / 密码值。

### 10.1 sisyphus 侧（运维 / 首次部署时配，业务仓无需关心）

由 `orchestrator/deploy/deploy-secrets.sh` 写入，之后用 `kubectl patch secret` 按需更新。

#### K8s Secret `orch-sisyphus-orchestrator`（namespace: `sisyphus`）

| Secret key | 注入 env | 用途 | 权限要求 | 缺失症状 |
|---|---|---|---|---|
| `bkd_token` | `SISYPHUS_BKD_TOKEN` | BKD REST API 鉴权（Coder-Session-Token） | BKD 控制台颁发 | orchestrator 起不来；所有 stage agent spawn 401 |
| `webhook_token` | `SISYPHUS_WEBHOOK_TOKEN` | BKD → orchestrator 入站 webhook 鉴权 | 任意强密码；不填时 deploy-secrets.sh 自动生成 | BKD 推事件返 401；REQ 永久卡在当前 stage |

#### K8s Secret `sisyphus-runner-secrets`（namespace: `sisyphus-runners`）

patch 方式及验证步骤见 §1c.2。

| Secret key | 注入 env | 用途 | 权限要求 | 缺失症状 |
|---|---|---|---|---|
| `gh_token` | `GH_TOKEN` | runner pod `git clone` 私有仓 | fine-grained PAT, Contents: Read-only；**必须覆盖所有接入业务仓 + 依赖的 integration repo（如 ttpos-arch-lab）** | clone 403/404；accept-env-up 拉 lab chart 失败；GitHub 报错文案可能是误导性"Write access not granted"（见 §1c.3） |
| `ghcr_user` | `SISYPHUS_GHCR_USER` | `docker login ghcr.io` 用户名 | GitHub 用户名或 org 名 | runner pod 私有镜像拉取失败（ImagePullBackOff） |
| `ghcr_token` | `SISYPHUS_GHCR_TOKEN` | `docker login ghcr.io` 密码 | classic PAT `read:packages` | runner pod 私有镜像拉取失败（ImagePullBackOff） |
| `kubeconfig`（文件挂载） | `KUBECONFIG=/workspace/.kubeconfig` | accept/staging 阶段 `helm install` + `kubectl exec` | k3s 集群访问权；entrypoint.sh 自动重写 server 地址为 in-cluster | accept-env-up helm / kubectl 命令报 "Unable to connect to the server" |

#### Orchestrator ConfigMap 可选 env

不配则对应功能静默降级，不影响核心状态机流程。

| Env var | 用途 | 权限要求 | 不配时效果 |
|---|---|---|---|
| `SISYPHUS_GITHUB_TOKEN` | orchestrator 侧 GitHub 操作：pr-ci-watch check-run 轮询、主动 CI dispatch（§6b）、PR base drift 监控、escalation incident issue | classic PAT `repo`；需开 incident issue 时额外 `Issues: Read-and-write` | dispatch / incident issue 跳过（warn log）；pr-ci-watch 轮询可能 rate limit |

### 10.2 业务仓侧（onboard 新业务仓时必配）

> 以下是**业务仓自己**（如 `ZonEaseTech/ttpos-flutter`）在 GitHub repo Settings 里配的 secret / vars。
> 由业务仓的 GHA dispatch workflow 消费，不是 sisyphus orchestrator 直接读。

| Name | 类型 | 位置 | 用途 | 权限要求 | 缺失症状 |
|---|---|---|---|---|---|
| `BUILD_REPO_PAT` | **Secret** | 业务仓 → Settings → Secrets and variables → Actions → Secrets | dispatch.yml 触发 `BUILD_REPO` CI 仓的 `repository_dispatch` | PAT，目标 CI 仓 Actions: write（classic PAT `repo`，或 fine-grained `Actions: read-and-write`） | dispatch 返 401；pr-ci-watch 无 check-run 可轮，永远 timeout（1800s 后 escalate） |
| `BUILD_REPO` | **Variable**（非 secret） | 业务仓 → Settings → Secrets and variables → Actions → Variables | dispatch.yml 中目标 CI 仓名 | 字符串，如 `ZonEaseTech/ttpos-ci` | dispatch `repository` 字段为空，workflow 起不来；无 check-run；pr-ci-watch timeout |

> `BUILD_REPO_PAT` / `BUILD_REPO` 是约定命名（来源 #275 / #310）。具体业务仓的
> dispatch.yml 可以用不同名字 —— 如果 pr-ci-watch 永远 timeout，先查这两项是否配齐。
...
```
□ runner secret GH_TOKEN（sisyphus-runner-secrets:gh_token）已把新仓加进 fine-grained PAT 可访问范围
□ GH_TOKEN 能通过 §1c.2 四步 API playbook（curl /user / /user/orgs / /repos/x / git ls-remote）
□ 新仓的 GHA workflow 在 PR 事件上跑并产生至少一条 check-run（否则 pr-ci-watch timeout）
