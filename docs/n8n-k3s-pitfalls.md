...
| BKD webhook 只订阅 `session.completed/failed` | 不要订阅 `issue.status.review`，会和 session 事件重复触发 |
| `$json.data` 不是 `$json.body` | fullResponse 模式下响应体在 data 字段 |

## 12. BKD Webhook 重复触发

**现象**：一个 issue 完成时，n8n 收到多次 webhook，创建了多个重复的下一阶段 issue。

**根因**：同时订阅了 `issue.status.review` 和 `session.completed`，一个 issue 完成时两个事件都触发。

**解决**：只订阅 `session.completed` 和 `session.failed`，不订阅 `issue.status.review`。

```json
{"events": ["session.completed", "session.failed"]}
```

**注意**：BKD webhook dispatcher 有 30 秒去重窗口，但不同 event type 不去重。
