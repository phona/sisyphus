...
8 张主表 + view。完整 DDL 见 [observability/schema.sql](../observability/schema.sql) 和 [orchestrator/migrations/](../orchestrator/migrations/) 0001~0010（0006 加 `verifier_decisions.audit` JSONB；0007 加 `event_seen.processed_at`；0009 加 `artifact_checks.attempts` + `flake_reason` 给 infra-flake retry 用；0010 加 `improver_runs` 给 improver-autopilot 用）。
