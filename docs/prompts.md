...
| [execute.md.j2](../orchestrator/src/orchestrator/prompts/execute.md.j2) | analyze（M17 全责交付：spec + 业务码 + push + 开 PR + unit test 都在一段里完成；execute-agent 自决是否开 BKD sub-issue 平行干） | `actions/start_execute.py`（直接入口）/ `actions/start_execute_with_finalized_intent.py`（intake 后入口） |
...
> `actions/fanout_specs.py` / `actions/create_dev.py` 已删；M17 起由 execute-agent
> 全责交付（写 spec + 写业务码 + push + 开 PR），如需并行 execute-agent 自决开 BKD sub-issue。
...
| stage | success | fail |
|---|---|---|
| execute | [verifier/execute_success.md.j2](../orchestrator/src/orchestrator/prompts/verifier/execute_success.md.j2) | [verifier/execute_fail.md.j2](../orchestrator/src/orchestrator/prompts/verifier/execute_fail.md.j2) |
...
| execute | `execute`（spawn 时已带）+ `REQ-xx` |
...
M17 起 execute-agent 必须 push `feat/REQ-x` 分支并真开 PR（`gh pr create`）。pr-ci-watch
