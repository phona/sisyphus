...
| [execute.md.j2](../orchestrator/src/orchestrator/prompts/execute.md.j2) | execute（M17 全责交付：spec + 业务码 + push + 开 PR + unit test 都在一段里完成；execute-agent 自决是否开 BKD sub-issue 平行干） | `actions/start_execute.py`（直接入口）/ `actions/start_execute_with_finalized_intent.py`（intake 后入口） |
