...
| fixer | `fixer` + `fixer:<dev\|spec\|manifest>` + `parent-stage:<stage>` + `parent-id:<verifier_issue_id>` |
...
dev-agent 开 PR 后必须把 `pr.number` 回写 `/workspace/.sisyphus/manifest.yaml`。否则 pr-ci-watch 跑到时找不到 PR。
