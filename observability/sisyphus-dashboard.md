...
核心 5 + 8 条：M7 原 5 条（Q1–Q5）基于 `artifact_checks`；M14e 追加 8 条（Q6–Q13）基于 `stage_runs` /
...
- **Visualization**：Table（列顺序：`req_id, state, last_stage, last_passed, stuck_min, recent_fail_24h, retry_analyze_count, last_checked_at, last_cmd`）
- **条件格式**：
  - `stuck_min > 30` 且 `last_passed = false` → 整行标红
  - `retry_analyze_count ≥ 3` → 该列底色黄
- **人工介入阈值**：
