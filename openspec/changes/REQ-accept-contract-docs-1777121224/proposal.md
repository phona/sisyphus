# REQ-accept-contract-docs-1777121224: docs(contracts): rename accept-up/down → ci-accept-env-up/down + Docker Compose template
...
- `orchestrator/src/orchestrator/actions/create_accept.py:60` 跑 `make ci-accept-env-up`
- `orchestrator/src/orchestrator/actions/teardown_accept_env.py:54` 跑 `make ci-accept-env-down`
- `orchestrator/src/orchestrator/prompts/accept.md.j2:25,29` 也提 `ci-accept-env-up` / `ci-accept-env-down`
- `orchestrator/docs/sisyphus-integration.md:81-82,184` 列的就是 `ci-accept-env-up` / `down`

**契约文档跟代码错位** —— 业务 repo 团队照 `docs/integration-contracts.md` 实现
`accept-up:` target 后接入 sisyphus，sisyphus 跑 `ci-accept-env-up`，
...
能直接抄的 `ci-accept-env-up:` / `ci-accept-env-down:` 实现，stdout 尾行也得吐
endpoint JSON。补一个 **docker-compose 模板**让接入路径更广。

## 根因

target 命名 M-x 之间发生过迁移（`accept-up` → `ci-accept-env-up`，与 ttpos-ci
标准的 `ci-` 前缀对齐），代码侧改完没回头同步契约文档。审计没发现是因为：
...
- §1 介绍段：`accept-up / accept-down` → `ci-accept-env-up / ci-accept-env-down`
- §2.3 表格：target 列改名；保留 "谁调 / 在哪跑 / 期望"，调用路径 `cd /workspace/integration/<repo-name> && make ci-accept-env-up`
- §3 标题 `accept-up 的 stdout JSON 契约` → `ci-accept-env-up 的 stdout JSON 契约`
- §3 实现建议代码块 target 名 `accept-up:` → `ci-accept-env-up:`
- §4.2 模板 `.PHONY: accept-up accept-down` → `.PHONY: ci-accept-env-up ci-accept-env-down`，
  recipe 头部同步
...
- §5 env 表 `SISYPHUS_STAGE` 行：`accept-up / accept-down` → `ci-accept-env-up / ci-accept-env-down`
- §8 排查清单第 4 条：`accept-up 失败` → `ci-accept-env-up 失败`

### Step 2：架构权威 `docs/architecture.md`

- §2 mermaid 节点 `make accept-up` / `make accept-down` 改名
- §5 角色分工表 "机械 checker" 行 `accept-up/down` → `ci-accept-env-up/down`
- §6 Stage 表 7a / 8 行：`make accept-up` / `make accept-down` 改名
- §7 数据流原语表 `make accept-up / accept-down` → `make ci-accept-env-up / ci-accept-env-down`
- §8 env 表 `SISYPHUS_STAGE` 行：`accept-up / accept-down` 改名
- §13 roadmap：`accept-up / accept-down 落到生产 lab` → `ci-accept-env-up / down 落到生产 lab`

### Step 3：`CLAUDE.md`

- "Stage 流" 一行 `make accept-up + agent ... + make accept-down 必跑` → `make ci-accept-env-up ... + make ci-accept-env-down 必跑`
...
- `grep -RIn 'ci-accept-env-up\|ci-accept-env-down'` 在 `docs/integration-contracts.md` ≥ 6 处命中（§2.3 / §3 / §4.2 / §4.2.2 / §5 / §8）
...
- **为什么不重命名 ENV var `SISYPHUS_STAGE` 取值** —— `SISYPHUS_STAGE=accept-env-up` / `accept-teardown`
  这两个**值** sisyphus 代码是真这么注的（`create_accept.py:52` / `teardown_accept_env.py:57`），
  跟 Makefile target 名是两码事；保持代码不动，文档对齐代码即可。
- **为什么不补 `dev-cross-check` 跑 docker-compose target 名校验** —— 不在本 REQ scope；
  ci-accept-env-up/down 是 integration repo 契约，sisyphus 自己不是 integration repo，
