...
### Requirement: integration repo Makefile target 契约 MUST 用 ci-accept-env-up / ci-accept-env-down 名
...
`ci-accept-env-up` and `ci-accept-env-down`. The system MUST NOT publish the
...
`ci-` prefixed names — publishing the old names would direct integrators to
implement a target that sisyphus never calls.

#### Scenario: AETN-S1 contract doc §2.3 lists ci-accept-env-up / down (renamed from accept-up / accept-down)

- **GIVEN** `docs/integration-contracts.md` is read
- **WHEN** the §2.3 integration repo Makefile target table is parsed
- **THEN** the table contains rows for `make ci-accept-env-up` and
  `make ci-accept-env-down`, AND no row mentions `make accept-up` or `make accept-down`

#### Scenario: AETN-S2 contract doc §3 stdout JSON section title is renamed

- **GIVEN** `docs/integration-contracts.md` is read
- **WHEN** §3's heading and body are scanned
- **THEN** the heading reads `## 3. ci-accept-env-up 的 stdout JSON 契约` (or contains
  the literal `ci-accept-env-up`), AND the implementation-suggestion makefile
  block declares the target as `ci-accept-env-up:` (not `accept-up:`)
...
- **THEN** the `.PHONY` line lists `ci-accept-env-up ci-accept-env-down` and
  the recipe headers are `ci-accept-env-up:` and `ci-accept-env-down:`
...
- **THEN** the "何时有" cell names `ci-accept-env-up / ci-accept-env-down` (or the
...
- **THEN** the bullet's title says `ci-accept-env-up 失败` (not `accept-up 失败`)
...
constraints: (a) declare `.PHONY: ci-accept-env-up ci-accept-env-down`;
(b) the `ci-accept-env-up` recipe MUST start the compose stack and emit a
single trailing JSON line containing an `endpoint` field on stdout, conforming
to §3's stdout JSON contract; (c) the `ci-accept-env-down` recipe MUST be
...
#### Scenario: AETN-S7 §4.2.2 declares both ci-accept-env-up and ci-accept-env-down as PHONY targets

- **GIVEN** `docs/integration-contracts.md` §4.2.2 docker-compose template
- **WHEN** the makefile block is parsed
- **THEN** there is a `.PHONY: ci-accept-env-up ci-accept-env-down` line, and
  both `ci-accept-env-up:` and `ci-accept-env-down:` appear as recipe headers

#### Scenario: AETN-S8 §4.2.2 ci-accept-env-up emits trailing endpoint JSON on stdout

- **GIVEN** the §4.2.2 template's `ci-accept-env-up` recipe
...
#### Scenario: AETN-S9 §4.2.2 ci-accept-env-down is best-effort idempotent

- **GIVEN** the §4.2.2 template's `ci-accept-env-down` recipe
- **WHEN** the recipe body is read
...
### Requirement: docs/architecture.md MUST 跟 contract 文档同步 ci-accept-env-up / ci-accept-env-down 命名

The architecture document `docs/architecture.md` SHALL refer to the integration
repo lab-up / lab-down targets exclusively as `ci-accept-env-up` / `ci-accept-env-down`
...
- **THEN** they contain `make ci-accept-env-up` and `make ci-accept-env-down` respectively,
...
- **THEN** the "产物 / 副作用" column names `make ci-accept-env-up` and
  `make ci-accept-env-down` respectively
...
- **THEN** the bullet says `ci-accept-env-up / ci-accept-env-down 落到生产 lab`,
  not the legacy names

### Requirement: README.md 与 CLAUDE.md MUST 同步 ci-accept-env-up / ci-accept-env-down 命名

The repo entry-point documents `README.md` and `CLAUDE.md` SHALL refer to the
integration repo lab-up / lab-down targets as `ci-accept-env-up` / `ci-accept-env-down`.
...
- **THEN** they contain `make ci-accept-env-up` and `make ci-accept-env-down`
  respectively
...
- **THEN** there are rows for `make ci-accept-env-up` and `make ci-accept-env-down`,
...
- **THEN** the line names `make ci-accept-env-up` and `make ci-accept-env-down`,
  not the legacy names
