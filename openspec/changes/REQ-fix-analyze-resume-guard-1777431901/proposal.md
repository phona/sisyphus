# Proposal: Add RESUME GUARD to execute prompt to prevent duplicate execution

## Problem

When execute-agent completes its work, BKD issue status synchronization may race or the agent may receive a SIGKILL. In such cases:

1. The BKD issue can briefly stay in "review" status (or a user may add a follow-up message to a done issue).
2. If the user wakes the agent again, it inherits the full history and restarts execute work.
...
Add a **RESUME GUARD** self-check section at the top of `execute.md.j2` (before Part A). The guard instructs the agent to verify objective facts via git/GitHub:
...
- `orchestrator/src/orchestrator/prompts/execute.md.j2` — add RESUME GUARD section
