...
### Requirement: execute.md.j2 SHALL include a RESUME GUARD self-check at the top of the prompt

The `execute.md.j2` prompt template MUST include a `RESUME GUARD` section placed before `Part A`. The section MUST instruct the agent to perform three objective checks using git and GitHub CLI:
...
If ANY of the three checks indicates the execute work is already complete, the agent MUST immediately stop execution and output a guard-triggered message without performing any further work.
...
- **WHEN** the execute-agent wakes and runs the RESUME GUARD checks
- **THEN** all three checks return negative
- **AND** the agent proceeds with normal execute work
...
- **AND** the agent MUST NOT perform any execute work
