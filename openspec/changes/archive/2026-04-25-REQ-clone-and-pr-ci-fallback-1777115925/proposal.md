# REQ-clone-and-pr-ci-fallback-1777115925: fix(start_execute + pr_ci_watch): server-side clone + refuse stale env fallback

## 问题

两条相互啮合的形状缺陷今天还撑着 sisyphus 的 happy path：

### 缺陷 A：start_execute 不做 server-side clone，agent 拿空 PVC 起跳

`actions/start_execute.py` 跟 `actions/start_execute_with_finalized_intent.py`
都只 `ensure_runner` 拉 Pod + PVC，然后立刻 follow-up `execute.md.j2` prompt
...
- 进 execute stage 时 `/workspace/source/` 是空的（PVC 新挂或上一 REQ 被 GC）。
  agent 必须先做 6 行 prompt 仪式才进入"实际写 spec / 写代码"——这 6 行
  里随便错一行（kubectl exec 命令拼错 / 仓名打字 / shallow refspec 漏带）
  整个 stage 都得回头修。
- M15 之后 sisyphus 不再起 spec / dev 子 agent，execute-agent 全责交付。
...
### Fix A：start_execute 接管 clone（server-side，不再委托 agent）

`start_execute` / `start_execute_with_finalized_intent` 在 `ensure_runner`
...
3. 退码非 0 → log + emit `VERIFY_ESCALATE`（不打 execute-agent；clone 起跳
   失败时 agent 也救不动）
4. 退码 0 / 没 repos：继续往下 dispatch agent

新 prompt 由 sisyphus 在 follow-up 里附一句 "/workspace/source/* 已经预
clone 好"（execute.md.j2 的 Part A.3 改成"如果 sisyphus 没替你 clone，再调
...
- **为什么不改成"不传 repos = clone 全部 PR-CI-historic-repo"** —— 全局
  ...
  （或者补加一条 `intent:execute:involved_repos:owner/repo` tag 给 router
...
- **start_execute 直接路径（无 intake）怎么办** —— ctx 里没
...
- 改 `actions/start_execute.py` / `actions/start_execute_with_finalized_intent.py`：
...
- 改 `prompts/execute.md.j2`：Part A.3 从硬要求改成 fallback；强调 ctx 已 clone
...
- 新增 `test_actions_start_execute.py`：覆盖 server-side clone 派发、
