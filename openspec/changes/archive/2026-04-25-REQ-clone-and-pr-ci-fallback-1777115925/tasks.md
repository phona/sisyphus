...
- [x] `orchestrator/src/orchestrator/prompts/execute.md.j2`：
...
- [x] `orchestrator/tests/test_actions_start_execute.py` (new)：
  - `test_start_execute_server_side_clones_when_involved_repos_present`：
    ctx 含 `involved_repos` → exec_in_runner 跑 sisyphus-clone-repos.sh
    + bkd.follow_up_issue 收到 prompt
  - `test_start_execute_skips_clone_when_no_involved_repos`：
    ctx 空 → 不调 exec_in_runner（直接路径兼容）
  - `test_start_execute_clone_failure_emits_verify_escalate`：
    exec_in_runner 退码非 0 → return 含 `emit: VERIFY_ESCALATE`
  - `test_start_execute_with_finalized_intent_clone_uses_involved_repos`：
