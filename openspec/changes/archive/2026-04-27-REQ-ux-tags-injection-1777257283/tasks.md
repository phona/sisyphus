...
- [x] `orchestrator/src/orchestrator/actions/start_execute.py`：PATCH tags 合并 `filter_propagatable_intent_tags(tags)`
- [x] `orchestrator/src/orchestrator/actions/start_execute_with_finalized_intent.py`：create tags 合并 `filter_propagatable_intent_tags(tags)`
...
- [x] `orchestrator/tests/test_actions_start_execute.py`：
  - `test_start_execute_forwards_user_hint_tags` —— tags 含 `repo:foo/bar` + `ux:fast-track` 时 PATCH 的 tags 含两者
  - `test_start_execute_strips_sisyphus_managed_tags` —— intent issue 上 stale `intent:execute` / `result:pass` / `pr:owner/repo#1` 不被转发
  - `test_start_execute_with_finalized_intent_forwards_hint_tags` —— intake 路径创新 issue 时同样转发
