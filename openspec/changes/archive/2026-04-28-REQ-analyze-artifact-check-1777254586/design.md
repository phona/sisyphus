# Design — execute 阶段 post-artifact-check

## 上下文

`spec_lint` 只调 `openspec validate` + `check-scenario-refs.sh`，关注
`specs/*/spec.md` 的格式。execute prompt 里要求的另外两件产物——
`proposal.md` / `tasks.md`——并没有任何机械检查，agent 完全可以挂
`execute` tag 把 issue 挪到 review 而**只生成空文件甚至完全不生成它们**，
sisyphus 仍会照常推进到 spec_lint。

REQ 的目标：在 execute 完成 → spec_lint 之前夹一层最便宜的契约校验，
...
| 可观测性 | artifact_checks 一行 stage=spec-lint，混在一起 | artifact_checks 单独一行 stage=execute-artifact-check，仪表盘可独立看 |
...
execute prompt 顶部 ✅ 清单包含：proposal.md、tasks.md、design.md（可选）、
...
| design.md | **不**机械校 | execute prompt 写"如需"，可选 |
...
```
EXECUTING --(EXECUTE_DONE)--> EXECUTE_ARTIFACT_CHECKING
  |- pass --(EXECUTE_ARTIFACT_CHECK_PASS)--> SPEC_LINT_RUNNING
  |- fail --(EXECUTE_ARTIFACT_CHECK_FAIL)--> REVIEW_RUNNING
                                              |- verify.pass --> EXECUTE_ARTIFACT_CHECKING --(PASS)--> SPEC_LINT_RUNNING
                                              |- verify.fix --> FIXER_RUNNING (fixer=spec)
                                              |- verify.escalate --> ESCALATED
```

`_verifier._PASS_ROUTING["execute_artifact_check"]` 走
`(EXECUTE_ARTIFACT_CHECKING, EXECUTE_ARTIFACT_CHECK_PASS)` —— `apply_verify_pass`
会先 CAS REVIEW_RUNNING → EXECUTE_ARTIFACT_CHECKING 再 emit PASS 让原主链
...
- 业务代码 diff 对 docs/spec-only REQ 是误伤；execute prompt 也没把它写成"必须"
...
1. **path 假设**：`/workspace/source/<repo>/openspec/changes/<REQ>/` 的形状假设
   sisyphus clone helper 已 stage（实际上 start_execute 已经替 agent 做完）。
