...
2. 缓存 miss：
   ...
   - 第一次成功后顺手回填 ctx 已知 sisyphus issue id 的 tag（典型场景：analyze
     issue 在创建时 PR 还不存在 → 第一次 verifier 创建时回填 execute issue tag）
...
`start_execute` / `start_execute_with_finalized_intent` 不调 helper（创 issue
时 PR 还不存在），但 stash `execute_issue_id` 进 ctx —— 这样后续 helper 第一次
discover 成功时能回填 execute issue 的 tag。
...
7. backfill_ids := gather_issue_ids_from_ctx(ctx)  # execute_issue_id, etc.
...
- 不改 BKD intake / execute prompt（agent 仍按当前 prompt 行事）
...
- **为什么 lazy discover 而不是 push-to-ctx 在 PR 开的时候** —— 没人主动告诉
  sisyphus "PR 开了"。execute-agent 在 prompt 里 push branch + gh pr create，但
...
- **为什么 backfill 只覆盖 execute_issue_id（+ 路径上其他已知 issue）** —— 只
  backfill ctx 里能直接读到的 id；不主动扫所有 BKD issue。简单 + 正确：第一次
  helper 命中时已知的就是 execute issue（intent → analyze）。
...
- 改 `orchestrator/src/orchestrator/actions/start_execute.py`：
  stash `execute_issue_id` 到 ctx（让后续 backfill 能找到）
