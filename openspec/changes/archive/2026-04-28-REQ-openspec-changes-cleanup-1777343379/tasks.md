...
- [x] Fix D: Add `_supersede_stale_openspec_changes()` to `start_execute.py` — moves same-slug old dirs to `_superseded/` before dispatch, fail-open
...
- [x] Fix ruff lint: update `test_actions_start_execute.py` exec_fn assertions to handle multiple exec_in_runner calls
...
- [x] `test_actions_start_execute_supersede.py` — SUPR-S1 (vN triggers supersede), SUPR-S2 (no-vN = no mv), SUPR-S3 (fail-open on exec error)
