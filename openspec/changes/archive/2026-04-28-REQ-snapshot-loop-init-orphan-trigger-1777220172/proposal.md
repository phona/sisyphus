# fix(snapshot): trigger INTENT_EXECUTE for missing req

## Why

The orchestrator's `intent:execute` entry path is webhook-only. When a user
adds the `intent:execute` tag to a fresh BKD issue, BKD posts
...
`(INIT, INTENT_EXECUTE) → EXECUTING + start_execute`.
...
- BKD issue still carries `intent:execute`,
- no `req_state` row,
...
trigger the missed `INTENT_EXECUTE` event itself.

## What Changes

- **Snapshot loop adds an orphan-recovery pass.** While iterating BKD
  issues per project, `sync_once` now also runs
  `_trigger_orphan_intent_execute`: for each issue that
  - has `intent:execute` in its tags,
  - does **not** have `execute` (i.e. has not yet entered the analyzing
  ...
  `req_state.insert_init` + `engine.step(INTENT_EXECUTE)` sequence the
  webhook handler would have.

- **No new state machine transitions, no new actions.** Recovery uses
  the existing `(INIT, INTENT_EXECUTE) → EXECUTING + start_execute`
...
- **Tests** (additions to `orchestrator/tests/test_snapshot.py`):
  ...
  - new `test_orphan_intent_execute_triggers_when_missing_req_state` —
    BKD issue with `intent:execute` tag, no REQ tag, no `req_state`
    row → `engine.step` is called with `cur_state=INIT` and
    `event=INTENT_EXECUTE`, and `req_state.insert_init` runs first.
  - new `test_orphan_intent_execute_skipped_when_already_in_req_state`
    — `req_state.get` returns a row → `engine.step` is not called.
  - new `test_orphan_intent_execute_skipped_when_analyze_tag_present`
    — issue already has both `intent:execute` and `execute` (mid- or
    post-flight) → no trigger.
  - new `test_orphan_intent_execute_skipped_when_status_done` — issue
...
- **Affected code**:
  - `orchestrator/src/orchestrator/snapshot.py` — refactor `sync_once`,
    add `_trigger_orphan_intent_execute` helper, change return type.
...
- **Bootstrap caveat**: orphan recovery walks
  ...
  subsequent missed `intent:execute` webhooks for that project recover
...
- **Risk**: low. `insert_init`'s `ON CONFLICT DO NOTHING` plus
  ...
  delayed real webhook. `start_execute` is `idempotent=True` and
  rebrands the BKD issue (adding `execute` and the `REQ-*` tag) so a
...
- **Out of scope**:
  - Recovery for `intent:intake` — same mechanism would work, but the
    ...
    to validate the execute path first.
