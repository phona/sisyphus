...
1. **verifier 子链**（M14b/c）
   - 任一上游 stage（execute_artifact_check / spec_lint / challenger / dev_cross_check
...
| VLT-S6 | `(EXECUTE_ARTIFACT_CHECKING, EXECUTE_ARTIFACT_CHECK_FAIL)` | dispatch `invoke_verifier_for_execute_artifact_check_fail`，state → `REVIEW_RUNNING` |
