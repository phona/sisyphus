# admin-stage-runs Specification

## Purpose
TBD - created by archiving change REQ-admin-esc-close-stagerun-1777338501. Update Purpose after archive.
## Requirements
...
#### Scenario: FESC-S1 EXECUTING state closes execute stage_run before escalate

- **GIVEN** a REQ in state EXECUTING with an open stage_run (ended_at IS NULL)
- **WHEN** `POST /admin/req/{req_id}/escalate` is called
- **THEN** `close_latest_stage_run(req_id, "execute", outcome="escalated", fail_reason="admin-force-escalate")` is called
