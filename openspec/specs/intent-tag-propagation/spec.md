...
- **GIVEN** input `["intent:execute", "REQ-foo-1234", "execute", "repo:phona/foo", "ux:fast-track", "result:pass", "pr:phona/foo#1"]`
- **WHEN** `filter_propagatable_intent_tags` is called
...
### Requirement: start_execute 必须把 user-hint tag 转发到 intent issue

The `start_execute` action MUST, when issuing the BKD `update_issue` PATCH that rewrites the intent issue's tags, append `filter_propagatable_intent_tags(tags)` to the base array `["execute", req_id]`.

#### Scenario: UTI-S11 start_execute forwards repo: tag through PATCH

- **GIVEN** webhook body.tags = `["intent:execute", "repo:phona/sisyphus"]` and `start_execute` is dispatched
- **WHEN** the rename PATCH is sent to BKD
- **THEN** the `update_issue` call's `tags` kwarg contains `"repo:phona/sisyphus"` and equals `["execute", "REQ-X", "repo:phona/sisyphus"]`

#### Scenario: UTI-S12 start_execute strips stale sisyphus-managed tags

- **GIVEN** webhook body.tags = `["intent:execute", "result:pass", "pr:phona/foo#1", "repo:phona/foo", "ux:fast-track"]`
- **WHEN** `start_execute` runs the rename PATCH
- **THEN** the `update_issue` call's `tags` kwarg equals `["execute", "REQ-X", "repo:phona/foo", "ux:fast-track"]` (no `intent:`, `result:`, `pr:` tags forwarded)

### Requirement: start_execute_with_finalized_intent 必须把 user-hint tag 转发到新建的 execute issue

The `start_execute_with_finalized_intent` action MUST, when calling `bkd.create_issue` to spawn the new execute sub-issue, set its `tags` argument to `["execute", req_id, *filter_propagatable_intent_tags(tags)]`. The `sisyphus` pipeline-identity tag is auto-injected by `BKDRestClient.create_issue` and MUST NOT be added explicitly here.

#### Scenario: UTI-S13 intake-path execute inherits hints from intake issue

- **GIVEN** webhook body.tags = `["sisyphus", "intake", "REQ-X", "result:pass", "repo:phona/foo", "ux:fast-track"]` (intake completion)
- **WHEN** `start_execute_with_finalized_intent` runs and creates the new execute issue
- **THEN** the BKD `create_issue` call's `tags` kwarg equals `["execute", "REQ-X", "repo:phona/foo", "ux:fast-track"]`
...
#### Scenario: UTI-S14 challenger inherits hints from execute issue

- **GIVEN** webhook body.tags = `["execute", "REQ-X", "repo:phona/foo", "ux:fast-track"]` and `pr_links.pr_link_tags(...)` returns `["pr:phona/foo#42"]`
...
- **GIVEN** webhook body.tags = `["execute", "REQ-X", "result:pass", "challenger", "intent:execute", "repo:phona/foo"]`
- **WHEN** `start_challenger` runs
