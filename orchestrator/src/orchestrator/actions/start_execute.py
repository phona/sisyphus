"""start_execute (v0.2)：intent:execute 入口。
...
REQ-clone-fallback-direct-execute-1777119520：直接 execute 路径（无 intake，
...
3. update-issue 把 intent issue 改名 [REQ-xxx] [EXECUTE] — <title> + tags=[execute, REQ-xxx]
4. follow-up-issue 发 execute prompt
...
async def start_execute(*, body, req_id, tags, ctx):
    ...
    async with BKDClient(settings.bkd_base_url, settings.bkd_token) as bkd:
        ...
        prompt = render(
            ...
            # REQ-pr-issue-traceability-1777218612: lets execute.md.j2 render
    ...
    await req_state.update_context(db.get_pool(), req_id, {
        "execute_issue_id": issue_id,
    })

    log.info("start_execute.done", req_id=req_id, issue_id=issue_id,
             cloned_repos=cloned_repos)
