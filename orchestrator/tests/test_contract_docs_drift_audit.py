...
def test_docs_s7_forward_migration_count_is_10():
    """DOCS-S7: orchestrator/migrations/ has exactly 10 forward migration files
    (0001..0010 contiguous; 0009 = artifact_checks_flake, 0010 = improver_runs)."""
...
def test_docs_s7_readme_migration_range_includes_0010():
    """DOCS-S7: README.md migration list reaches 0010 (improver_runs)."""
    readme = (REPO_ROOT / "README.md").read_text()
    assert re.search(r"0010|001[0-9]", readme), (
        "README.md must reference latest migration 0010"
    )


def test_docs_s7_claude_migration_range_includes_0010():
    """DOCS-S7: CLAUDE.md migration list reaches 0010."""
    claude = (REPO_ROOT / "CLAUDE.md").read_text()
    assert re.search(r"0010|001[0-9]", claude), (
        "CLAUDE.md must reference latest migration 0010"
    )


def test_docs_s7_observability_md_migration_range():
    """DOCS-S7: docs/observability.md states migration range includes 0010."""
    obs_md = (REPO_ROOT / "docs" / "observability.md").read_text()
    assert re.search(r"0010|0001.{0,15}0010", obs_md), (
        "docs/observability.md must state migration range goes to 0010"
    )
