...
async def test_ert_s2_intaking_intake_pass_enters_executing(monkeypatch) -> None:
    """ERT-S2: (INTAKING, INTAKE_PASS) → EXECUTING via start_execute_with_finalized_intent.
    Intake clarification succeeded; finalized intent is forwarded to the execute phase."""
    from orchestrator.actions import REGISTRY
    from orchestrator.state import Event, ReqState
...
_ESCALATE_CASES = [
    ...
    pytest.param(
        ...
        id="ERT-S5-executing_verify_escalate",
    ),
    pytest.param(
