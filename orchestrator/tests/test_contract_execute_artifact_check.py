"""Challenger contract tests for REQ-execute-artifact-check-1777254586.

Black-box contracts derived exclusively from:
  openspec/changes/REQ-execute-artifact-check-1777254586/specs/execute-artifact-check/spec.md

Written by: challenger-agent (M18 — independent of dev implementation)

Scenarios covered:
  AAC-S1  EXECUTING/EXECUTE_DONE → EXECUTE_ARTIFACT_CHECKING + create_execute_artifact_check
          AND EXECUTE_ARTIFACT_CHECKING/PASS → SPEC_LINT_RUNNING + create_spec_lint
          AND create_execute_artifact_check is callable via actions REGISTRY
  AAC-S2  EXECUTE_ARTIFACT_CHECKING/FAIL → REVIEW_RUNNING + invoke_verifier_for_execute_artifact_check_fail
          + verifier _STAGES contains "execute_artifact_check"
  AAC-S3  EXECUTE_ARTIFACT_CHECKING in SESSION_FAILED self-loop
  ...
  AAC-S7  create_execute_artifact_check pass writes artifact_checks + emits PASS
  AAC-S8  create_execute_artifact_check non-zero exit emits FAIL + still writes artifact_checks
...
def test_aac_s1_execute_done_routes_to_artifact_check_then_spec_lint() -> None:
    """AAC-S1: (EXECUTING, EXECUTE_DONE) MUST route through EXECUTE_ARTIFACT_CHECKING
    ...
    """
    t1 = decide(ReqState.EXECUTING, Event.EXECUTE_DONE)
    assert t1 is not None, (
        "AAC-S1: (EXECUTING, EXECUTE_DONE) MUST be a registered transition"
    )
    assert t1.next_state is ReqState.EXECUTE_ARTIFACT_CHECKING, (
        "AAC-S1: EXECUTE_DONE MUST move into EXECUTE_ARTIFACT_CHECKING, not "
        "directly into SPEC_LINT_RUNNING. Got next_state="
        f"{t1.next_state.value!r}"
    )
    assert t1.action == "create_execute_artifact_check", (
        "AAC-S1: EXECUTE_DONE transition action MUST be 'create_execute_artifact_check'. "
        f"Got {t1.action!r}"
    )

    t2 = decide(ReqState.EXECUTE_ARTIFACT_CHECKING, Event.EXECUTE_ARTIFACT_CHECK_PASS)
    assert t2 is not None, (
        "AAC-S1: (EXECUTE_ARTIFACT_CHECKING, EXECUTE_ARTIFACT_CHECK_PASS) MUST be "
        "a registered transition"
    )
    assert t2.next_state is ReqState.SPEC_LINT_RUNNING, (
        "AAC-S1: EXECUTE_ARTIFACT_CHECK_PASS MUST move to SPEC_LINT_RUNNING. "
        f"Got {t2.next_state.value!r}"
    )
    assert t2.action == "create_spec_lint", (
        "AAC-S1: EXECUTE_ARTIFACT_CHECK_PASS action MUST be 'create_spec_lint'. "
        f"Got {t2.action!r}"
    )

    # The orchestrator dispatches actions by name via REGISTRY; if
    # create_execute_artifact_check is not registered, the state machine
    # would fail at runtime even though the transition is defined.
    from orchestrator.actions import REGISTRY
    assert "create_execute_artifact_check" in REGISTRY, (
        "AAC-S1: 'create_execute_artifact_check' MUST be registered in actions.REGISTRY "
        "so the orchestrator can dispatch it when EXECUTE_DONE fires"
    )
...
def test_aac_s2_fail_routes_to_verifier_with_dedicated_handler() -> None:
    """AAC-S2: (EXECUTE_ARTIFACT_CHECKING, FAIL) → REVIEW_RUNNING +
    invoke_verifier_for_execute_artifact_check_fail; the action is registered;
    _STAGES contains the new stage so invoke_verifier accepts it.
    """
    t = decide(ReqState.EXECUTE_ARTIFACT_CHECKING, Event.EXECUTE_ARTIFACT_CHECK_FAIL)
    ...
    assert t.action == "invoke_verifier_for_execute_artifact_check_fail", (
        f"AAC-S2: FAIL action MUST be the dedicated verifier handler. Got {t.action!r}"
    )

    from orchestrator.actions import REGISTRY
    assert "invoke_verifier_for_execute_artifact_check_fail" in REGISTRY, (
        "AAC-S2: invoke_verifier_for_execute_artifact_check_fail MUST be registered "
        "in the actions REGISTRY"
    )

    from orchestrator.actions._verifier import _STAGES
    assert "execute_artifact_check" in _STAGES, (
        "AAC-S2: '_verifier._STAGES' MUST contain 'execute_artifact_check' so "
...
def test_aac_s3_session_failed_self_loop_covers_new_state() -> None:
    """AAC-S3: EXECUTE_ARTIFACT_CHECKING MUST appear in the SESSION_FAILED self-loop
    so a runner-side crash during the artifact check is funneled through escalate
    rather than silently bouncing.
    """
    t = TRANSITIONS.get((ReqState.EXECUTE_ARTIFACT_CHECKING, Event.SESSION_FAILED))
    assert t is not None, (
        "AAC-S3: (EXECUTE_ARTIFACT_CHECKING, SESSION_FAILED) MUST be in TRANSITIONS"
    )
    assert t.next_state is ReqState.EXECUTE_ARTIFACT_CHECKING, (
        "AAC-S3: SESSION_FAILED self-loop MUST keep state on "
        "EXECUTE_ARTIFACT_CHECKING (action 'escalate' decides whether to truly "
...
def _get_build_cmd(req_id: str = "REQ-X") -> str:
    ...
    from orchestrator.checkers import execute_artifact_check as c
...
def test_aac_s5_build_cmd_checks_proposal_tasks_spec_checkbox_feat_branch() -> None:
    ...
    assert 'git fetch origin "feat/REQ-X"' in cmd, (
        "AAC-S5: cmd MUST fetch origin feat/REQ-X to obtain the execute branch"
    )
...
async def test_aac_s7_action_pass_writes_artifact_then_emits_pass(monkeypatch) -> None:
    ...
    'execute-artifact-check' AND return dict containing emit=PASS event.
    """
    ...
    assert out["emit"] == "execute-artifact-check.pass", (
        "AAC-S7: emit MUST be execute-artifact-check.pass on success"
    )
    ...
    assert insert_calls[0][1] == "execute-artifact-check", (
        f"AAC-S7: artifact_checks stage MUST be 'execute-artifact-check'. "
...
async def test_aac_s8_action_nonzero_emits_fail_with_exit_code(monkeypatch) -> None:
    ...
    fake_result = CheckResult(
        ...
        stdout_tail="",
        stderr_tail="=== FAIL execute-artifact-check: ... ===\n",
    ...
    assert out["emit"] == "execute-artifact-check.fail", (
        "AAC-S8: emit MUST be execute-artifact-check.fail on non-zero exit"
    )
