...
Plus the explicitly stated lower-level contract from the Requirement body:
  ...
  - ``actions/start_analyze.py`` and
    ``actions/start_analyze_with_finalized_intent.py`` MUST call it after
...
def _normalize():
    ...
    assert callable(fn), (
        ...
        "entry point shared by start_analyze and "
...
def test_cbor_normalize_returns_new_dict_does_not_mutate_input() -> None:
    ...
    Callers in ``start_analyze`` rely on the input dict (often built from
...
# ─── Wiring: BOTH start_analyze call sites MUST invoke normalize ─────────────
#
...
# full async start_analyze pipeline (DB / BKD / k8s / dispatch_slugs mocks).


def test_cbor_start_analyze_module_invokes_normalize_base_overrides() -> None:
    """start_analyze.py MUST reference ``normalize_base_overrides``.

    Spec: ``actions/start_analyze.py ... MUST call it after merging
    ...
    assert "normalize_base_overrides" in src, (
        "CBOR wiring: start_analyze.py MUST reference normalize_base_overrides "
...
def test_cbor_start_analyze_with_finalized_intent_module_invokes_normalize_base_overrides() -> None:
    """start_analyze_with_finalized_intent.py MUST reference ``normalize_base_overrides``.
    ...
    assert "normalize_base_overrides" in src, (
        "CBOR wiring: start_analyze_with_finalized_intent.py MUST reference "
