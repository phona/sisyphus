...
def test_ert_s9_sweep_covers_exactly_79():
    """Sanity: TRANSITIONS 必须正好 79 条 —— 加 / 减 transition 时这条 fail 提醒
    ...
    增至 49；后续 REQ 再增 1 条至 50；REQ-hotfix-mode-1777420843 新增 hotfix 精简
    流水线（19 显式 + 7 SESSION_FAILED self-loop）增至 79。"""
    assert len(state_mod.TRANSITIONS) == 79, (
        f"expected 79 transitions, got {len(state_mod.TRANSITIONS)}; "
