...
@pytest.mark.asyncio
async def test_start_execute_with_finalized_intent_creates_new_issue(monkeypatch):
    from orchestrator.actions import start_execute_with_finalized_intent as mod

    fake = make_fake_bkd()
    fake.create_issue = AsyncMock(return_value=FakeIssue(id="execute-new-1"))
    ...
    assert out["execute_issue_id"] == "execute-new-1"
...
@pytest.mark.asyncio
async def test_start_execute_with_finalized_intent_forwards_hint_tags(monkeypatch):
    """REQ-ux-tags-injection: 创新 execute issue 时 hint tag 跟着进 tags 数组。"""
    from orchestrator.actions import start_execute_with_finalized_intent as mod
...
@pytest.mark.asyncio
async def test_start_execute_missing_finalized_intent_escalates(monkeypatch):
...
@pytest.mark.asyncio
async def test_start_execute_none_ctx_escalates(monkeypatch):
