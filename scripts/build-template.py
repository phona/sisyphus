...
nodes.append(http_node(
    ...
    follow_up_body(
        ...
        "- 看清需求要改什么 / 怎么改 / 联动哪些仓库\n"
        "- 定高层方案（选型、取舍、风险）\n"
        "- 产 tasks 骨架（只粗粒度，细节留给下游 spec-agent）\n"
        "\n"
        "## 产出 3 个文件\n"
        ...
        "   - 影响范围：改哪些模块 / 文件 / 仓库\n"
        "   - 支持性判定：支持 / 不支持 / 需澄清\n"
        "   - **frontmatter 必写 `repos:` 列表**，例：`repos: [ubox-crosser]`（单仓）或 `[repoA, repoB]`（多仓）\n"
        "2. openspec/changes/$REQ/design.md\n"
        "   - 高层方案（不给具体 OpenAPI/SQL/UI 代码）\n"
        "   - 接口形态：REST / gRPC / CLI / UI（仅描述形态，不定细节）\n"
        "   - 依赖与风险\n"
        "3. openspec/changes/$REQ/tasks-outline.md\n"
        "   - [contract] 粗粒度：需要覆盖哪些 API / 数据结构契约（**只列清单，不写 yaml**）\n"
        "   - [dev] 粗粒度：要实现哪些模块（**不写代码**）\n"
        "   - [accept] 粗粒度：要验哪些用户行为（**不写 Given/When/Then**）\n"
        "\n"
        "## 禁止越权\n"
        "- **不要写 contract.spec.yaml**（contract-spec agent 的活）\n"
        "- **不要写 specs/FEATURE-*.md**（accept-spec agent 的活）\n"
        "- **不要写 tasks.md 的具体任务**（dev-spec agent 的活）\n"
        "你只是立框架，不做细化。\n"
        "\n"
        "## 完成时\n"
        "- 3 个文件 commit + push 到 feat/$REQ\n"
        "- update-issue tags（保留原有 analyze + $REQ）：\n"
        "  - 追加 `repo:<name>`（proposal.md 的 repos 列表里每个一个 tag，单仓就一个）\n"
        "  - 不支持：追加 `decision:unsupported`\n"
        "  - 需澄清：追加 `decision:needs-clarify`\n"
        "- move review\n"
        "\n"
        "n8n 收到 session.completed 后会 fan-out 3 个 spec-agent（dev / contract / accept）分别深化。",
        91),
...
nodes.append(http_node(
    ...
    follow_up_body(
        ...
        "\n"
        "### 若 specStage == contract-spec\n"
        "- **读** openspec/changes/$REQ/proposal.md + design.md + tasks-outline.md（analyze 已产）\n"
        "- **产** openspec/changes/$REQ/contract.spec.yaml — OpenAPI/IDL 风格，精确定义每个接口/数据形态\n"
        "- **产** openspec/changes/$REQ/specs/contract/*.md — 契约测试用例（Given input, assert output）\n"
        "- **性质**：LOCKED。dev-agent 不得改；ci-integration 按此验证\n"
        "- commit + push 到 feat/$REQ\n"
        "\n"
        "### 若 specStage == accept-spec\n"
        "- **读** proposal + design（analyze 已产）\n"
        "- **产** openspec/changes/$REQ/specs/FEATURE-A-*.md — Acceptance Scenario 列表\n"
        "  每个场景 Given/When/Then 格式，描述**用户视角**的验证点（非测试代码）\n"
        "- **性质**：LOCKED。accept-agent 在验收阶段按这个跑\n"
        "- commit + push 到 feat/$REQ\n"
        "\n"
        "### 若 specStage == dev-spec\n"
        "- **读** proposal + design + tasks-outline + **contract.spec.yaml**（若 contract-spec 已产）\n"
        "- **产** openspec/changes/$REQ/tasks.md — 可执行任务拆解（Stage: backend / frontend / ...，每条带验收点）\n"
        "- **产** openspec/changes/$REQ/implementation-plan.md — 实现思路（算法选型、模块拆分、数据流）\n"
        "- **性质**：**不锁定**，dev-agent 执行时可调整\n"
        "- commit + push 到 feat/$REQ\n"
        "\n"
        "## 硬规则\n"
        "- 禁改 analyze 产出的 proposal.md / design.md / tasks-outline.md\n"
        "- 禁写其他 spec-agent 的产物（各管各的）\n"
        "- pre-commit ACL 会拦越权改动\n"
        "\n"
        "## 完成时\n"
