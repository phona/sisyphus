...
nodes.append(http_node(
    ...
    follow_up_body(
        ...
        "   - tasks.md 的 `## Stage: implementation` section（待你填细节 + 勾完成）\n"
        "2. 从 feat/$REQ 拉 stage/$REQ-dev 子分支\n"
        "3. **填 tasks.md 的 Stage: implementation section**：先把 analyze 留的 `- [ ] TODO: ...`\n"
        "   换成具体任务清单（依然 `- [ ]` 开头）\n"
        "4. 实现业务代码（internal/ cmd/）+ 同目录 unit test\n"
        "5. 通过 mcp__aissh-tao__exec_run 在 vm-node04 跑 `go vet` / `go build` / `make ci-unit-test` 验证\n"
        "6. **每完成一条就把对应 checkbox 改成 `- [x]`**（这是 dev-agent 的工作产物之一，\n"
        "   PR 审查的人要看这个 tick 率判断完成度）。最终 commit 里所有\n"
        "   Stage: implementation 下的 checkbox 都应该是 `[x]`。如果某条做不完、放弃，\n"
        "   改成 `- [~] <原因>` 标注。\n"
        "7. ONE 干净 commit + push 到 stage/$REQ-dev\n"
        "8. move review\n"
        "\n"
